    ...
    public async Task<CategoriaDTO> Crear(CategoriaDTO modelo)
    {
      try
      {
        ...
        if (rspModelo.IdCategoria != 0)
          return _mapper.Map<CategoriaDTO>(dbModelo);
        else
          throw new TaskCanceledException("Categoria no creada");
      }
      catch (Exception ex)
      {
    ...
    public async Task<bool> Editar(CategoriaDTO modelo)
    {
      try
      {
        ...
        if (fromDbModelo == null)
        {
          ...
          if (!respuesta)
            throw new TaskCanceledException("Categoria no editada");
          return respuesta;
        }
        else
        {
          throw new TaskCanceledException("Categoria no encontrada");
        }
      }
      catch (Exception ex)
      {
    ...
    public async Task<bool> Eliminar(int id)
    {
      try
      {
        ...
        if (fromDbModelo != null)
        {
          ...
          if (!respuesta)
            throw new TaskCanceledException("Categoria no eliminada");
          else
            return respuesta;
        }
        else
